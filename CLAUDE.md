...
3. **`ocracy.LEVELS` is not the granularity levels.** At the package root the bare name `ocracy.LEVELS` is the readiness ladder from `ocracy.status`: `("all", "implemented", "set_up", "tested")`. The granularity tuple `("page", "block", "paragraph", "line", "word", "char")` is `ocracy.GRANULARITY_LEVELS` (= `ocracy.base.LEVELS`), and `ocracy.STATUS_LEVELS` is an unambiguous alias for the readiness one. Use the explicit names; `tests/test_levels_names.py` pins all three.
