...
Note: computing **tested** runs real OCR — for *remote* backends that means a real
...
def is_set_up(backend_id: str) -> bool:
    ...
    """
    from ocracy import registry
    from ocracy.install import check
...
def is_tested(backend_id: str, *, image: Any = None) -> bool:
    ...
    try:
        report = validate_adapter(backend_id, image=image)
    except Exception:
        return False
...
def backend_info(
    ...
    *,
    ...
    if to_test and test_image is None:
        from ocracy.make_backend import make_test_image
    ...
    for i in ids:
        ...
        if i in to_test:
            info["tested"] = is_tested(i, image=test_image)
        else:
            info["tested"] = None
        out[i] = info
    return out
...
def backend_ids(
    ...
    *,
    ...
    img = test_image if test_image is not None else make_test_image()
    return [i for i in set_up if is_tested(i, image=img)]
...
_COLUMNS = [
    ("Name", lambda d: _trunc(d.get("name") or d.get("id"), 30)),
    ...
    ("Best for", lambda d: _trunc(d.get("best_for"), 46)),
    ("Website", lambda d: _trunc(d.get("homepage"), 48)),
]
