...
- :class:`ServiceCollection` — a lazy mapping ``backend_id -> ServiceHandle``,
  also reachable by attribute (``services.tesseract``).
- :class:`ServiceHandle` — a per-backend namespace exposing the normalized
  ``read`` method, the backend's ``config``/``info``, and the raw ``adapter``.

This gives three escalating levels of control::

    ocracy.ocr(img)                          # 1. simple facade (default backend)
    ocracy.services.tesseract.read(img, lang="fra")   # 2. pick a backend explicitly
    ocracy.services.tesseract.adapter        # 3. raw engine adapter, full control

Mirrors the sibling ``denote`` facade's service layer.
"""
...
class ServiceHandle:
    """Per-backend access point: normalized ``read`` + native ``adapter``."""
    ...
    def read(self, image, **kwargs):
        """Read text from ``image`` with this backend. Returns an ``OcrResult``."""
        return self.adapter.read(image, **kwargs)
