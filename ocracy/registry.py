...
An *implemented* backend is a subpackage of :mod:`ocracy.backends` that ships
two things:
...
- ``adapter.py`` defining an ``Adapter`` class whose ``read(image, **kwargs)``
  returns an :class:`ocracy.base.OcrResult`.
...
PRIMARY_CAPABILITY = "read"


def _discover_backends() -> None:
    """Scan ``ocracy.backends.*`` for ``BACKEND_CONFIG`` dicts (runs once)."""
    global _discovered
    if _discovered:
        return
    _discovered = True
    ...
    try:
        import ocracy.backends as backends_pkg
    except ImportError:  # pragma: no cover
        return

    for _importer, modname, ispkg in pkgutil.iter_modules(
        backends_pkg.__path__, prefix="ocracy.backends."
    ):
        if not ispkg:
            continue
...
def get_default_backend(
    ...
    if not candidates:
        raise ValueError(
            ...
            f"Install/implement one, e.g. pip install 'ocracy[tesseract]'."
        )
